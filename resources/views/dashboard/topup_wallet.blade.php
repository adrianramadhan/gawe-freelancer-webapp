    ...
    <div class="py-12 bg-gray-900 min-h-screen">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                ...
                <div>
                    <h3 class="text-gray-200 text-xl font-bold mb-5">Send Payment to:</h3>
                    <div class="flex flex-row gap-x-10">
                        <div>
                            <p class="text-slate-400 text-sm">Bank</p>
                            <h3 class="text-gray-200 text-xl font-bold">Angga Capital</h3>
                        </div>
                        <div>
                            <p class="text-slate-400 text-sm">No Account</p>
                            <h3 class="text-gray-200 text-xl font-bold">08123092093</h3>
                        </div>
                        <div>
                            <p class="text-slate-400 text-sm">Account Name</p>
                            <h3 class="text-gray-200 text-xl font-bold">Gawebro Indonesia</h3>
                        </div>
                    </div>
                </div>
                ...
                <h3 class="text-gray-200 text-xl font-bold">Confirm Topup Wallet</h3>
                <form method="POST" action="{{route('dashboard.wallet.topup.store')}}" enctype="multipart/form-data">
                    @csrf

                    <div>
                        <x-input-label for="amount" :value="__('amount')" class="text-gray-200"/>
                        <x-text-input id="amount" class="block mt-1 w-full bg-gray-700 text-gray-200" type="text" name="amount" :value="old('amount')" required autofocus autocomplete="amount" />
                        <x-input-error :messages="$errors->get('amount')" class="mt-2 text-red-500" />
                    </div>

                    <div class="mt-4">
                        <x-input-label for="proof" :value="__('proof')" class="text-gray-200"/>
                        <x-text-input id="proof" class="block mt-1 w-full bg-gray-700 text-gray-200" type="file" name="proof" required autofocus autocomplete="proof" />
                        <x-input-error :messages="$errors->get('proof')" class="mt-2 text-red-500" />
                    </div>
